...
SUBSCRIBES TO:
    /loggerhead/raspicam_node/image: Source image topic

PUBLISHES TO:
    /loggerhead/balloon/image_balloon : image with detected balooon and search window
    /loggerhead/balloon/image_mask : masking
    /loggerhead/balloon/point_balloon : balloon position in a dimensional values wrt. camera frame
...
class BalloonDetector:
    ...
    def __init__(self, thr_min, thr_max, blur=15, balloon_params=None, detection_window=None):
        ...
        self.image_pub = rospy.Publisher("/loggerhead/balloon/image_balloon",Image,queue_size=1)
        self.mask_pub = rospy.Publisher("/loggerhead/balloon/image_mask",Image,queue_size=1)
        print (">> Publishing position to topic point_balloon")
        self.balloon_pub  = rospy.Publisher("/loggerhead/balloon/point_balloon",Point,queue_size=1)

        self.bridge = CvBridge()
        self.image_sub = rospy.Subscriber("/loggerhead/raspicam_node/image",Image,self.callback)
        print ("<< Subscribed to topic /loggerhead/raspicam_node/image")
    ...
    def callback(self,data):
        ...
        if cols > 60 and rows > 60 :
            ...
            for i, keyPoint in enumerate(keypoints):
              ...
              #  time.sleep(1) #team turtlebot addition
                break 

            fps = 1/(time.time()-self._t0)
            self._t0 = time.time()

def main(args):
    balloon_min = (16,39,125)
    balloon_max = (33,185,220)
    ...
    x_min   = 0.05
    x_max   = 0.95
    y_min   = 0.45 #team TB - decreased the height above horizon to speed detction
    y_max   = 0.85 #team TB - decreased the height below horizon to speed detction
    ...
    ic = BalloonDetector(balloon_min, balloon_max, blur, params, detection_window)
