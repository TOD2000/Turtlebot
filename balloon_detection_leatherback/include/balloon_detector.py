...
https://github.com/tizianofiorenzani/ros_tutorials/blob/master/opencv/include/blob_detector.py
...
def balloon_detect(image,  # -- The frame (cv standard)
                ...
                balloon_params=None,  # -- blob parameters (default None)
                search_window=None,
                # -- window where to search as [x_min, y_min, x_max, y_max] adimensional (0.0 to 1.0) starting from
                # top left corner
                imshow=False
                ):
...
def draw_keypoints(image,  # -- Input image
                   ...
                   imshow=True  # -- show the result  #edited by team_turtlebot FROM FALSE TO
                   ):
    ...
    if imshow:
        # Show keypoints
        cv2.imshow("Balloon Detections", im_with_keypoints)
...
if __name__ == "__main__":
    ...
    # --- Define HSV limits
    balloon_min = (70, 0, 160)
    balloon_max = (100, 250, 250)
    ...
    image_list = []
    image_list.append(cv2.imread("frame0000.jpg"))
