...
SUBSCRIBES TO:
    /leatherback/raspicam_node/image: Source image topic

PUBLISHES TO:
    /leatherback/balloon/image_balloon : image with detected balooon and search window
    /leatherback/balloon/image_mask : masking
    /leatherback/balloon/point_balloon : balloon position in a dimensional values wrt. camera frame
...
class BalloonDetector:
    ...
    def __init__(self, thr_min, thr_max, blur=15, balloon_params=None, detection_window=None):
        ...
        self.image_pub = rospy.Publisher("/leatherback/balloon/image_balloon",Image,queue_size=1)
        self.mask_pub = rospy.Publisher("/leatherback/balloon/image_mask",Image,queue_size=1)
        print (">> Publishing position to topic point_balloon")
        self.balloon_pub  = rospy.Publisher("/leatherback/balloon/point_balloon",Point,queue_size=1)

        self.bridge = CvBridge()
        self.image_sub = rospy.Subscriber("/leatherback/raspicam_node/image",Image,self.callback)
        print ("<< Subscribed to topic /leatherback/raspicam_node/image")
    ...
    def callback(self,data):
        ...
        if cols > 60 and rows > 60 :
            ...
            cv2.imshow("Balloon Hunter", mask)
...
def main(args):
    balloon_min = (122,64,59)
    balloon_max = (142,255,255)
